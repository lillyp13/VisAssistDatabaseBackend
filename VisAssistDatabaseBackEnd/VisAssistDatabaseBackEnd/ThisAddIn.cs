...
namespace VisAssistDatabaseBackEnd
{
    public partial class ThisAddIn
    {
        ...
        public Dictionary<string, MateSelection> m_odictMatesInSelection = new Dictionary<string, MateSelection>();
        public List<string> m_olstMatesMated = new List<string>();
        public bool m_bIsPageDuplicating = false;
        public bool m_bSyncedDB = false;
        public bool m_bAskWhereToCutTo = true;
        public Visio.Selection m_ovSelection;
        public string m_sLastUndoScope = "";
        public List<string> m_olstWireIDs = new List<string>();
        public List<string> m_olstWirePairIDs = new List<string>();
        public List<string> m_olstPagesinProcessofDeleting = new List<string>();
                                ...
                                if (ovShape != null)
                                {
                                    if (ovShape.ID != 0)
                                    {
                                        ...
                                        if (!m_pendingShapeIds.Contains(sKey))
                                        {
                                            ...
                                            if (!m_olstMatesMated.Contains(sKey))
                                            {
                                                ...
                                                if (m_odictMatesInSelection.ContainsKey(sKey))
                                                {
                                                    //we need to update their ids...
                                                    MateSelection oMateSelection = m_odictMatesInSelection[sKey];
                                                ...
                                                if (!m_bIsCuttingShape)
                                                {
                                                    if (!m_bIsPageDuplicating)
                                                    {
                                                        ...
                                                        string sMateKey = WireMateUtilities.IsMateInSelection(m_ovSelection, ovShape, out ovMateShape);
                                                        if (sMateKey != "")
                                                        {
                                                            ...
                                                            m_odictMatesInSelection.Add(sKey, oMateSelection);
                                                        }
                                        ...
                                        foreach (Visio.Shape ovShapeToCheck in m_ovSelection)
                                        {
                                            if (ovShapeToCheck.CellExists["User.Class", 0] == -1)
                                            {
                                                if (ovShapeToCheck.Cells["User.Class"].get_ResultStr(0) == "SmartWire")
                                                {
                                                    ...
                                                    m_olstWireIDs.Add(sWireID);
                                                    m_olstWirePairIDs.Add(ovShape.Cells["User.WirePairID"].get_ResultStr(0));
                                                }
                                            }
                                        }
                                    }
                ...
                case (short)((short)Visio.VisEventCodes.visEvtDel + (short)Visio.VisEventCodes.visEvtPage):
                    {
                        ...
                        else
                        {
                            ...
                            if(!m_olstPagesinProcessofDeleting.Contains(sPageToDeleteFromUndo))
                            {
                                m_olstPagesinProcessofDeleting.Add(sPageToDeleteFromUndo);
                            }
                ...
                case (short)((short)VisioEvents.visEvtAdd + (short)Visio.VisEventCodes.visEvtPage):
                    {
                        ...
                        try
                        {
                            ...
                            if(m_olstPagesinProcessofDeleting.Contains(sPageName))
                            {
                        ...
                        else
                        {
                            ...
                            Dictionary<string, Visio.Page> odictPagesToDuplicate = WireMateUtilities.DoesPageContainWireMates(ovPage);
